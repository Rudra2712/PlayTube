...
function EditChannelInfo() {
  ...
  useEffect(() => {
    if (user) {
      setFormData({
        ...
        timezone: user.timezone || "UTC+05:30"
      });
    }
  }, [user]);
  ...
  const handleSubmit = () => {
    // Note: Your backend doesn't have endpoints for updating username/description/timezone
    // You would need to add these endpoints to your controller
    // For now, this will show a message
    toast.info("Channel info update functionality needs backend endpoints. Currently only name/email and images are supported.");
    console.log("Channel info to update:", formData);
  };

  const characterLimit = 275;
  const remainingChars = characterLimit - (formData.description?.length || 0);
