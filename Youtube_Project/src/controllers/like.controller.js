...
const getLikedVideos = asyncHandler(async (req, res) => {
    ...
    // Extract video data from likes and filter out null videos
    const videos = likedVideos
        .filter(like => like.video) // Filter out likes where video is null
        .map(like => ({
            ...like.video.toObject(),
            likeId: like._id,
...
// NEW: Get video like status for a specific video
const getVideoLikeStatus = asyncHandler(async (req, res) => {
    ...
    return res.status(200).json(
        new ApiResponse(200, result, "Video like status fetched successfully")
    );
});
