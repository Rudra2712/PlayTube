...
const generateAccessAndRefreshTokens = async (userId) => {
  try {
    ...
    //   }
    console.error("Real error while saving user:", error);
    throw new ApiError(500, "Error generating tokens");
  }
};
...
const refreshAccessToken = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    throw new ApiError(
      401,
      error?.message || "Invalid refresh token format or signature",
    );
  }
});
...
const getUserChannelProfile = asyncHandler(async (req, res) => {
  ...
  const channel = await User.aggregate([
    ...
    {
      $lookup: {
        //counted the subscribers through channel
        from: "subscriptions",
        localField: "_id",
    ...
    {
      $lookup: {
        // count the channels subscribed to by the user through subscriber
        from: "subscriptions",
        localField: "_id",
    ...
    {
      $addFields: {
        //adding the fields to the channel
        subscribersCount: {
          //counting the subscribers
          $size: "$subscribers",
        },
        channelsSubscribedToCount: {
          //counting the channels subscribed to by the user
          $size: "$subscribedTo",
        },
        isSubscribed: {
